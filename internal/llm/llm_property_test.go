...
func TestProperty_ValidMessageRoutingToLLM(t *testing.T) {
	...
	properties.Property("valid messages are routed to LLM provider", prop.ForAll(
		func(modelID, userMessage string) bool {
...
func TestProperty_LLMResponseDelivery(t *testing.T) {
	...
	properties.Property("LLM responses are delivered correctly", prop.ForAll(
		func(modelID, responseContent string, tokensUsed uint16) bool {
			// Skip if required fields are empty
			if modelID == "" || responseContent == "" {
				return true
			}
...
func TestProperty_ResponseTimeTracking(t *testing.T) {
	...
	properties.Property("response time is tracked for all LLM requests", prop.ForAll(
		func(modelID string, delayMs uint8) bool {
			// Skip if required fields are empty or delay is too large
			if modelID == "" || delayMs > 200 {
				return true
			}
...
func TestProperty_LLMRequestContextInclusion(t *testing.T) {
	...
	properties.Property("LLM requests include session context and user message", prop.ForAll(
		func(modelID string, messageCount uint8) bool {
			// Skip if required fields are empty or message count is too large
			if modelID == "" || messageCount == 0 || messageCount > 10 {
				return true
			}
...
func TestProperty_StreamingResponseForwarding(t *testing.T) {
	...
	properties.Property("streaming responses are forwarded in real-time", prop.ForAll(
		func(modelID string, chunkCount uint8) bool {
			// Skip if required fields are empty or chunk count is too large
			if modelID == "" || chunkCount == 0 || chunkCount > 10 {
				return true
			}
...
func TestProperty_LLMBackendRetryLogic(t *testing.T) {
	...
	properties.Property("LLM backend failures trigger retry with exponential backoff", prop.ForAll(
		func(modelID string, failureCount uint8) bool {
			// Skip if required fields are empty or failure count is too large
			if modelID == "" || failureCount == 0 || failureCount > 3 {
				return true
			}
...
func TestProperty_ModelSelectionPersistence(t *testing.T) {
	...
	properties.Property("selected model is used for all subsequent requests", prop.ForAll(
		func(modelID1, modelID2 string, requestCount uint8) bool {
			// Skip if required fields are empty or models are the same
			if modelID1 == "" || modelID2 == "" || modelID1 == modelID2 || requestCount == 0 || requestCount > 5 {
				return true
			}
		...
		gen.Identifier(),      // modelID1
		gen.Identifier(),      // modelID2
...
func TestProperty_TokenUsageTrackingAndStorage(t *testing.T) {
	...
	properties.Property("token usage is tracked and accumulated correctly", prop.ForAll(
		func(modelID string, requestCount uint8, tokensPerRequest uint16) bool {
			// Skip if required fields are empty or counts are too large
			if modelID == "" || requestCount == 0 || requestCount > 10 || tokensPerRequest == 0 {
				return true
			}
