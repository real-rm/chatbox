...
import { MOCK_SESSIONS, MOCK_EMPTY_SESSIONS, TEST_JWT_TOKEN } from '../helpers/mock-data.js';
...
test.describe('Sessions Page', () => {
  test.describe('with sessions', () => {
    ...
    test('shows admin-assisted badge on relevant sessions', async ({ page }) => {
      await page.goto(`/sessions.html?token=${TEST_JWT_TOKEN}`);
    ...
    test('navigates to new chat when clicking New Chat button', async ({ page }) => {
      await page.goto(`/sessions.html?token=${TEST_JWT_TOKEN}`);
  ...
  test.describe('empty state', () => {
    ...
    test('shows empty state when no sessions exist', async ({ page }) => {
      ...
      await expect(page.locator('#empty-state')).toContainText('No chat sessions yet');
    });
  ...
  test.describe('error handling', () => {
    test('shows error when no token is provided', async ({ page }) => {
      ...
      await expect(page.locator('#status-text')).toContainText('No authentication token');
    });
    ...
    test('shows error when API returns error', async ({ page }) => {
      ...
      await expect(page.locator('#connection-status')).toBeVisible();
      await expect(page.locator('#status-text')).toContainText('Failed to load sessions');
    });
  });
  ...
  test.describe('JWT token storage', () => {
    test('stores token from URL in sessionStorage', async ({ page }) => {
      ...
      const storedToken = await page.evaluate(() => sessionStorage.getItem('jwt_token'));
