...
function initializeEventListeners() {
  ...
  // Auto-refresh toggle
  document
    .getElementById("auto-refresh-status")
...
async function loadMetrics() {
  try {
    ...
    if (currentFilters.dateStart)
      params.append("start_time", currentFilters.dateStart);
...
async function loadSessions() {
  try {
    ...
    if (currentFilters.adminAssisted)
      params.append("admin_assisted", currentFilters.adminAssisted);
...
function updateMetricsDisplay(metrics) {
  ...
  document.getElementById("avg-concurrent").textContent = metrics.avg_concurrent
    ? metrics.avg_concurrent.toFixed(1)
    : "0.0";
...
function updateSessionsDisplay(sessions) {
  ...
  // Build table HTML with data attributes instead of inline onclick (XSS prevention)
  tbody.innerHTML = sessions
    .map((session) => {
      ...
      return `
        <tr>
            <td>
                <span class="user-link" data-user-id="${safeUserId}">
                    ${safeUserId}
                </span>
            </td>
            ...
            <td>
                ${
                  session.is_active
                    ? `<button class="btn-small" data-takeover-session="${safeSessionId}">Takeover</button>`
                    : "-"
                }
            </td>
        </tr>`;
  ...
  // Bind click handlers via addEventListener instead of inline onclick (XSS prevention)
  tbody.querySelectorAll("[data-user-id]").forEach((el) => {
    el.addEventListener("click", () => showUserSessions(el.dataset.userId));
  });
  tbody.querySelectorAll("[data-takeover-session]").forEach((el) => {
    el.addEventListener("click", () =>
      takeoverSession(el.dataset.takeoverSession),
    );
  });
}
...
function applyFilters() {
  currentFilters.userID = document
    .getElementById("filter-user-id")
    .value.trim();
