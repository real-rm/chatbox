...
class SessionList {
  ...
  async loadSessions() {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}${PATH_PREFIX}/sessions`, {
        method: "GET",
        headers: {
  ...
  openSession(sessionID) {
    ...
    const chatUrl = `chat.html?token=${encodeURIComponent(token)}&session_id=${encodeURIComponent(sessionID)}${apiParam}`;
    window.location.href = chatUrl;
  }

  // Create New Session — end any active session first so the server allows a fresh one
  async createNewSession() {
    ...
    if (active) {
      try {
        await fetch(`${API_BASE}${PATH_PREFIX}/sessions/${active.id}/end`, {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        });
      } catch (err) {
        console.warn("Could not end active session:", err);
      }
    }

    const apiParam = _qp.get("api") ? `&api=${_qp.get("api")}` : "";
    const chatUrl = `chat.html?token=${encodeURIComponent(token)}&new=1${apiParam}`;
    window.location.href = chatUrl;
  }
